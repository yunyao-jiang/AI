...
- The Docker deployment installs `dlib-bin` first and then installs `face-recognition` with `--no-deps` to avoid high-memory source builds of `dlib` on cloud platforms.
